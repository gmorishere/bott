...
local function run(msg, matches)
  ...
  if matches[1] == 'en' and matches[3] == 'gp' then
  ...
  -- Enable a plugin
  if matches[1] == 'en' and is_sudo(msg) then --after changed to moderator mode, set only sudo
  ...
  if matches[1] == 'dis' and matches[3] == 'gp' then
    local plugin = matches[2]
    local receiver = get_receiver(msg)
  ...
  -- Disable a plugin
  if matches[1] == 'dis' and is_sudo(msg) then --after changed to moderator mode, set only sudo
  ...
  if matches[1] == 're' and is_sudo(msg) then --after changed to moderator mode, set only sudo
    return reload_plugins(true)
  end
end
...
return {
  ...
  usage = {
      moderator = {
          "plug dis [plugin] gp : disable plugin only this chat.",
          "plug en [plugin] gp : enable plugin only this chat.",
          },
      sudo = {
          "plug : list all plugins.",
          "plug en [plugin] : enable plugin.",
          "plug dis [plugin] : disable plugin.",
